...
const DEFAULTS = {
  title: "Abednego Morara | Mechanical Engineer, Fabricator & Digital Systems Builder",
  description:
...
const PAGE_SEO = {
  ...
  services: {
    ...
    description:
      "Engineering and fabrication services including CAD and mechanical design, CNC production support, machine servicing and repair, and custom digital systems.",
  },
  booking: {
...
function SEO({ page = "home", title, description, path = "/", image = DEFAULTS.image, noindex = false, type = "website" }) {
  const config = PAGE_SEO[page] || DEFAULTS;
  const finalTitle = title || config.title || DEFAULTS.title;
  const finalDescription = description || config.description || DEFAULTS.description;
  ...
  useEffect(() => {
    ...
    const professionalService = { "@type": "ProfessionalService", "@id": `${SITE_URL}/#professional-service`, name: "Abednego Morara Engineering & Digital Services", url: SITE_URL, provider: { "@id": PERSON_ID }, areaServed: ["Kenya", "Worldwide"], serviceType: ["Mechanical Engineering", "Mechanical Design", "CNC Fabrication", "Machine Maintenance", "Machine Reliability", "Production Management Systems", "Web Development"] };

    const graph = [person, website, professionalService];
  ...
  }, [canonicalUrl, finalDescription, finalTitle, imageUrl, noindex, normalizedPath, type]);
