...
const capabilities = [
  { icon: <FaDraftingCompass />, number: "01", title: "Design & Engineering", text: "From concept to fabrication-ready CAD, with the real manufacturing process considered from day one." },
  { icon: <FaBolt />, number: "02", title: "CNC & Fabrication", text: "Laser cutting, sheet metal, welding, bending and production support built around getting the job right." },
  { icon: <FaCogs />, number: "03", title: "Machines & Reliability", text: "Troubleshooting, maintenance and practical interventions that keep workshop equipment moving." },
  { icon: <FaLaptopCode />, number: "04", title: "Digital Systems", text: "Websites and production tools that turn scattered information into clear, useful workflows." },
];
    ...
    <main>
      <section className="hero">
        ...
        <div className="hero-content">
          ...
          <Reveal delay={150}>
            <p className="hero-description">Mechanical engineering meets the workshop floor and the digital world. I design, fabricate, troubleshoot and build systems that solve real problems.</p>
          </Reveal>
          <Reveal delay={220}>
        ...
        <Reveal className="hero-portrait" delay={180}>
          <div className="portrait-frame">
            <img src="/images/abednego.jpeg" alt="Abednego Morara" />
          </div>
        </Reveal>
        ...
        <div className="capability-strip-inner">
          {capabilities.map((item, i) => (
            <RainbowCard key={item.title} className="capability-strip-card">
              ...
              <h3>{item.title}</h3>
              <p>{item.text}</p>
              <NavLink to="/services">See what I can do <FaArrowRight /></NavLink>
            </RainbowCard>
          ))}
        </div>
      </section>
      ...
      <section className="home-capabilities">
        ...
        <div className="capabilities-grid">
          {capabilities.map((item, i) => <Reveal key={item.title} delay={i * 70}><RainbowCard className="capability-card"><div className="card-top"><span>{item.number}</span><div className="capability-icon">{item.icon}</div></div><h3>{item.title}</h3><p>{item.text}</p><NavLink to="/services">See what I can do <FaArrowRight /></NavLink></RainbowCard></Reveal>)}
        </div>
      </section>
      ...
      <section className="home-work">
        ...
        <Reveal className="featured-work edge-box">
          <div className="featured-work-image"><img src="/images/hydraulic-bending-machine-3.jpeg" alt="Hydraulic sheet-metal bending machine" /><span className="image-badge">FEATURED / 01</span></div>
          <div className="featured-work-content"><p className="eyebrow">MECHANICAL FABRICATION</p><h3>Hydraulic Sheet-Metal<br />Bending Machine</h3><p>A practical machine built by combining a locally fabricated structure with sourced hydraulic and tooling components.</p><div className="spec-strip"><div><strong>1.2m</strong><span>Working length</span></div><div><strong>3mm</strong><span>Sheet capacity</span></div><div><strong>2×</strong><span>Hydraulic jacks</span></div></div><NavLink to="/projects" className="outline-link">Inspect project <FaArrowRight /></NavLink></div>
        </Reveal>
      </section>
