...
const projects = [
  ["E-Water Meter Boxes", "SHEET METAL", "/images/project-meter.jpeg", "Meter box components developed around repeatable sheet-metal production and assembly."],
  ["Consumer Boxes", "ELECTRICAL ENCLOSURE", "/images/project-consumer.jpeg", "CNC-cut and bent enclosure components designed for practical production."],
  ["Tent Fabrication", "STRUCTURAL FABRICATION", "/images/project-tent.jpeg", "Structural fabrication work translating drawings into repeatable physical assemblies."],
  ["Curved Axis Beam", "MECHANICAL DESIGN", "/images/project-beam.jpeg", "A specialised mechanical beam combining design thinking with fabrication realities."],
  ["Real-Time Fabrication Tracker", "SOFTWARE / PRODUCTION", "/images/project-tracker.jpeg", "A production management system connecting jobs, people, material, stages and progress."],
];
...
function Projects() {
  return <main>
    ...
    <section className="projects-section">
      ...
      <div className="projects-grid">{projects.map(([title,category,img,text],i)=><Reveal key={title} delay={i*60}><article className={`project-card edge-box ${i===4?"project-card-wide":""}`}><div className="project-image"><img src={img} alt={title} /><span className="project-number">0{i+2}</span></div><div className="project-info"><span>{category}</span><h3>{title}</h3><p>{text}</p><FaExternalLinkAlt className="project-arrow" /></div></article></Reveal>)}</div>
